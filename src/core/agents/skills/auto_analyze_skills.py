            ...
            issues.append(
                {
                    "severity": "high",
                    "column": col,
                    "issue": f"Más del 50% de valores nulos ({null_percent * 100:.1f}%)",
                }
            )
            ...
            issues.append(
                {
                    "severity": "medium",
                    "column": col,
                    "issue": f"Valores nulos presentes ({null_percent * 100:.1f}%)",
                }
            )
            ...
            issues.append(
                {
                    "severity": "high",
                    "column": col,
                    "issue": "Columna constante (1 solo valor único)",
                }
            )
            ...
            issues.append(
                {
                    "severity": "medium",
                    "column": col,
                    "issue": "Alta correlación detectada (>0.85) con otra variable",
                }
            )
            ...
            issues.append(
                {
                    "severity": "low",
                    "column": col,
                    "issue": "Alta cardinalidad en variable categórica (posible ID)",
                }
            )
...
def recommend_transformations(
    session: AnalysisSession, profile: dict | None = None, issues: list | None = None
) -> dict:
    if not session.has_data() or session.current_df is None:
        return {"error": "No hay datos en la sesión."}
    ...
    for issue in issues:
        ...
        if "nulos" in issue.get("issue").lower():
            ...
            if col_info.get("type") in ["object", "category"]:
                recommendations.append(
                    {
                        "column": col,
                        "recommendation": "Imputar con la moda o añadir categoría 'Desconocido'",
                        "reason": issue["issue"],
                    }
                )
            else:
                recommendations.append(
                    {
                        "column": col,
                        "recommendation": "Imputar con la mediana",
                        "reason": issue["issue"],
                    }
                )
        if "constante" in issue.get("issue").lower():
            recommendations.append(
                {
                    "column": col,
                    "recommendation": "Eliminar columna",
                    "reason": "No aporta varianza al modelo",
                }
            )
        if "alta correlación" in issue.get("issue").lower():
            recommendations.append(
                {
                    "column": col,
                    "recommendation": "Eliminar variable colineal o aplicar PCA",
                    "reason": "Evitar multicolinealidad",
                }
            )
    ...
    for col, info in cols_info.items():
        if info.get("skewness") and abs(info.get("skewness")) > 2:
            recommendations.append(
                {
                    "column": col,
                    "recommendation": "Aplicar transformación logarítmica",
                    "reason": f"Alta asimetría detectada (Skewness = {info.get('skewness'):.2f})",
                }
            )
...
def recommend_models(session: AnalysisSession, target_col: str | None = None) -> dict:
    if not session.has_data() or session.current_df is None:
        return {"error": "No hay datos en la sesión."}
    ...
    if not target_col:
        ...
        recommendations.append(
            {
                "task": "Segmentación / Clustering",
                "models": ["K-Means", "DBSCAN", "Hierarchical Clustering"],
                "reason": "No se especificó variable objetivo.",
            }
        )
    else:
            ...
            recommendations.append(
                {
                    ...
                    "reason": f"Variable objetivo '{target_col}' detectada como categórica ({unique_target} clases).",
                }
            )
        else:
            recommendations.append(
                {
                    ...
                    "reason": f"Variable objetivo '{target_col}' detectada como continua.",
                }
            )
...
def generate_business_insights(session: AnalysisSession) -> dict:
    ...
    if not numeric_df.empty:
        ...
        for col in numeric_df.columns:
            ...
            if mean != 0:
                ...
                if abs(cv) > 1.5:
                    insights.append(
                        f"La variable '{col}' muestra una variabilidad extrema respecto a su media (Coeficiente de variación alto). Se sugiere investigar la causa de estos picos o caídas abruptas."
                    )
            ...
            for col in upper.columns:
                for idx in upper.index:
                    ...
                    if pd.notna(val) and 0.7 < val < 0.99:
                        insights.append(
                            f"Existe una fuerte relación lineal ({val * 100:.1f}%) entre '{col}' y '{idx}'. Esta relación puede ser clave para la toma de decisiones predictivas."
                        )
    ...
    return {
        "result": insights
        if insights
        else ["No se detectaron insights heurísticos obvios en esta muestra."]
    }
...
def auto_analyze(session: AnalysisSession, target_col: str | None = None) -> dict:
    if not session.has_data() or session.current_df is None:
        return {"error": "No hay datos en la sesión."}
    ...
    transformations = recommend_transformations(session, profile=profile, issues=issues).get(
        "result", []
    )

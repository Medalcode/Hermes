...
def test_recommend_transformations(sample_session):
    ...
    assert any(
        rec["column"] == "constant_col" and "Eliminar" in rec["recommendation"] for rec in recs
    )
...
def test_generate_business_insights(sample_session):
    ...
    assert any(
        "salary" in insight.lower() and "variabilidad" in insight.lower() for insight in insights
    )
